package org.jplus.pojo;

/**
 * @author wb
 * @date 2019/10/28 - 13:39
 */
public class QTJXGZ {
    private Integer qtjxgzid;
    private String gh;
    private Integer nd;
    private Float ddgzl;
    private Integer bfxsrs;
    private Integer zdqnjsrs;

    public Integer getZdqnjsrs() {
        return zdqnjsrs;
    }

    public void setZdqnjsrs(Integer zdqnjsrs) {
        this.zdqnjsrs = zdqnjsrs;
    }

    private Integer xdrcpyfa;
    private Integer kcdgms;
    private Integer sydgms;

    public Float getDdgzl() {
        return ddgzl;
    }

    public void setDdgzl(Float ddgzl) {
        this.ddgzl = ddgzl;
    }

    public void setGh(String gh) {
        this.gh = gh;
    }


    public Integer getBfxsrs() {
        return bfxsrs;
    }

    public void setBfxsrs(Integer bfxsrs) {
        this.bfxsrs = bfxsrs;
    }

    public Integer getXdrcpyfa() {
        return xdrcpyfa;
    }

    public void setXdrcpyfa(Integer xdrcpyfa) {
        this.xdrcpyfa = xdrcpyfa;
    }

    public Integer getKcdgms() {
        return kcdgms;
    }

    public void setKcdgms(Integer kcdgms) {
        this.kcdgms = kcdgms;
    }

    public Integer getSydgms() {
        return sydgms;
    }

    public void setSydgms(Integer sydgms) {
        this.sydgms = sydgms;
    }

    public float getQtgzl() {
        return qtgzl;
    }

    public void setQtgzl(float qtgzl) {
        this.qtgzl = qtgzl;
    }

    private float qtgzl;

    public Integer getQtjxgzid() {
        return qtjxgzid;
    }

    public void setQtjxgzid(Integer qtjxgzid) {
        this.qtjxgzid = qtjxgzid;
    }

    public String getGh() {
        return gh;
    }

    public Integer getNd() {
        return nd;
    }

    public void setNd(Integer nd) {
        this.nd = nd;
    }

}
